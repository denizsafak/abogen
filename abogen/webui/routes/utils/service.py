...
def submit_job(pending: PendingJob) -> str:
    ...
    job = service.enqueue(
        ...
        camb_model=getattr(pending, "camb_model", "mars-flash"),
        camb_voice_id=getattr(pending, "camb_voice_id", 147320),
        camb_api_key=getattr(pending, "camb_api_key", None),
        camb_language=getattr(pending, "camb_language", "en-us"),
